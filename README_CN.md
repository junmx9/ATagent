...
ATAgent 是一个**轻量级 Agent 框架**，帮助开发者为自己的应用快速添加自然语言控制能力。
...
- **支持语音输入**：开发者可自行集成语音识别，将语音转为文本后传入框架，完整的语音控制体验开箱可用。
...
开发者在 `config/actions.json` 中定义应用的全部动作结构——这是框架的核心，AI 的理解与执行都在此结构内进行：
...
框架在连续两次澄清未能解决问题后，自动提示用户可以使用这种方式。
...
会话状态由 `sessionId` 标识，框架默认内存存储，开发者可替换为 Redis 等持久化方案。
...
- **框架只提供 API，UI 和话术由开发者掌控**：内置合理默认值，需要时可全量覆盖。
